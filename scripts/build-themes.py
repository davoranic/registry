...
def build(path):
    ...
    tokens = theme["tokens"]
    name = theme["name"]
    # SCOPED selectors: a theme paints the page OR any single container.
    # data-theme="<name>" on any element themes that subtree — which is what
    # lets a registry show one component in two characters side by side.
    root_sel = f':root[data-theme="{name}"], [data-theme="{name}"]'
    dark_sel = (f':root[data-theme="{name}"][data-mode="dark"], '
                f'[data-theme="{name}"][data-mode="dark"]')
    out = [f"/* GENERATED by scripts/build-themes.py from {path.name} — DO NOT EDIT.\n"
           ...
           f'   Apply with data-theme="{name}" on <html> or on ANY container;\n'
           f'   data-mode="dark" and data-density="..." scope the same way. */\n']
    ...
    if by_density:
        ...
        for density, values in by_density.items():
            out.append(f'[data-theme="{name}"][data-density="{density}"] {{\n'
