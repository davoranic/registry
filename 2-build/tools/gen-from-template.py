...
def generate(tpl, system):
    col = load(ROOT / "columns" / f"{tpl['component']}.{system}.json")
    ...
    out = ROOT / "out/gen"; out.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    systems = sorted(p.stem.split(".")[1] for p in (ROOT / "columns").glob(f"{component}.*.json"))
