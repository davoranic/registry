...
citation unit for this system.

> **CORRECTION (found while building the dialog matrix).** This page
> previously claimed M3's tokens-only clone "doesn't carry the resolved
> family string" and treated Roboto as an implied default. **That was
> wrong.** `md-ref-typeface` carries both families as literals, in *both*
> editions:
> `versions/v0_192/_md-ref-typeface.scss:17-18` → `'brand': (Roboto)`,
> `'plain': (Roboto)`; `versions/latest/sass/_md-ref-typeface.scss:19,21` →
> `$brand: Roboto`, `$plain: Roboto`.
> Consequence: M3 font-family cells are **`[S]`, not `[R]`**. Several
> earlier matrices (calendar, button, tooltip, input) flagged the Roboto
> family `[R]` on the strength of this page's incorrect claim; those cells
> are sourced and should be re-graded on their next pass. Recorded here
> rather than silently edited into each matrix so the provenance of the
> correction stays visible — and as a caution that a wrong entry in a
> foundations page propagates into every component that trusts it instead
> of re-grepping.
